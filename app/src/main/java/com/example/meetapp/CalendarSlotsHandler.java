...
class CalendarSlotsHandler {
    ...
    void setButtonsIdForListeners(int daysNum, Context context) {
        ...
        for (int i = 0; i < daysNum; i++) {
            int morningBtnId = context.getResources().getIdentifier(dayPrefix + i + MORNING_PREFIX,
                    idDefType, context.getPackageName());
            buttonsIdForListeners.put(morningBtnId, MORNING);
            int afternoonBtnId = context.getResources().getIdentifier(dayPrefix + i + AFTERNOON_PREFIX,
                    idDefType, context.getPackageName());
            buttonsIdForListeners.put(afternoonBtnId, AFTERNOON);
            int eveningBtnId = context.getResources().getIdentifier(dayPrefix + i + EVENING_PREFIX,
                    idDefType, context.getPackageName());
    ...
    private void timeSlotComparator(List<Map.Entry<TimeSlot, Integer>> list) {
    ...
    private void chooseTopSelections(ArrayList<String> topSuggestions, int iterationNumber) {
        for (int i = 0; i < iterationNumber; i++) {
            ...
            currentSlot = String.format(context.getString(R.string.timeSlotStringFormat), currentSlot,
                    topSlot.getDate(), topSlot.getHour(), getSelectionNumber(topSlot), membersAmount);
    ...
    private void displayInitSelections(HashMap<String,Long> userCalendar, HashMap<String,Long> allUsersCalendar) {
        ...
        for (TimeSlot ts : slotSelections.keySet()) {
            ...
            ts.getButton().setBackground(SlotBackgroundSetter.setBackGroundColorAndBorder(bgColor,
                    userChooseMark, context));
        }
    }
